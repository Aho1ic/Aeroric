...
- **描述**：Claude Code 与 Codex CLI 当前版本的 hook 事件 / payload 字段 / 配置方式 / 信任机制全量对照，以及 Aeroric 实际订阅哪些事件、为何这样映射，面向 hook 链路开发与排查。
- **标签**：`hooks`, `claude-code`, `codex`, `event-watcher`, `session-discovery`, `input-required`, `references`

> 本文是**外部能力 + Aeroric 用法**的对照参考。两边官方都在快速迭代，事件清单以官方文档为准（[Claude](https://code.claude.com/docs/en/hooks) / [Codex](https://developers.openai.com/codex/hooks)），本文聚焦「Aeroric 依赖的子集 + 易踩的字段/信任差异」。截至 2026-06。

---

## 1. 整体流程

Aeroric 不直接调 agent API，而是给 agent 注入一个共享 hook 脚本，脚本把事件落盘成 `events.jsonl`，后端轮询解析后驱动任务状态。

```mermaid
flowchart TD
    subgraph install["安装期 (一次性)"]
      A["hooks::ensure_installed()"] --> B["写 ~/.aeroric/hooks/aeroric-hook.mjs"]
      A --> C["写 ~/.aeroric/hooks/claude-settings.json<br/>(Claude 专用, --settings 传入)"]
      A --> D["注入 ~/.codex/config.toml<br/>(marker 块内联 [[hooks.X]])"]
    end
    subgraph run["任务运行期"]
      E["pty.rs 启动 agent<br/>注入 AERORIC_TASK_ID/EVENT_DIR/AGENT"] --> F["agent 触发 hook event"]
      F --> G["aeroric-hook.mjs 读 stdin JSON<br/>守卫: 无 AERORIC_TASK_ID 则 exit 0"]
      G --> H["追加一行到<br/>~/.aeroric/events/&lt;task_id&gt;/events.jsonl"]
...
src-tauri/src/aeroric-hook.mjs
  └─ pick(payload, ...keys)         ← 跨 agent 字段名多 key 兜底
src-tauri/src/pty.rs
  ├─ build_codex_cmd() + --dangerously-bypass-hook-trust  (use_hooks 时,须在 -- / resume 前)
  ├─ build_claude_cmd() + --settings <aeroric_claude_settings_path>
  └─ AERORIC_TASK_ID / AERORIC_EVENT_DIR / AERORIC_AGENT 环境变量注入
...
## 2. Aeroric 落盘的事件行（跨 agent 统一契约）

每次 hook 触发，`aeroric-hook.mjs` 把不同 agent 的异构 payload 归一成同一行写入 `events.jsonl`：

```json
{
  "ts": 1733300000000,
  "task_id": "<AERORIC_TASK_ID>",
  "agent": "claude | codex",
...
Claude 当前版本的 hook 事件远超 Aeroric 所需（含 `PostToolBatch`、`FileChanged`、`TaskCreated`、`SubagentStart`、`PermissionDenied`、`StopFailure`、`SessionEnd` 等数十个）。**Aeroric 仅订阅 6 个**（`hooks.rs::CLAUDE_EVENTS`）：

| 事件 | Aeroric 用途 | 触发时机要点 |
|------|-----------|-------------|
...
| `Stop` | 置 `input_required`（本轮结束、等输入） | AI 完成一轮响应。**Aeroric 主要依赖项**，不靠 `Notification` 兜底 |
| `SubagentStop` | 不主动 emit，交 PTY exit monitor | 子代理结束，主代理仍在工作 |

Claude 端关键能力：
- **配置合并语义**：`hooks` 是数组型 key，跨 settings 来源 **concat + 按 command 去重**，不会覆盖用户 hook。Aeroric 据此走 `--settings <自有文件>`，**完全不改用户 `~/.claude/settings.json`**。
- **退出码**：`exit 0` = 成功（stdout 可回传 JSON 决策/上下文）；`exit 2` = 阻断（对 `Stop`/`SubagentStop` 意味"继续工作"，须查 `stop_hook_active` 防死循环）。Aeroric 脚本永远 `exit 0`，纯观测不干预。
- **handler 类型**：`command` / `http` / `mcp_tool` / `prompt` / `agent`。Aeroric 用 `command`。

### 3.2 Codex 事件支持

Codex hooks 从 0.99（AfterAgent）→ 0.114（实验引擎，含 SessionStart/Stop）演进，**0.124.0（2026-04-23）起 stable 且默认开启**。当前官方事件：`SessionStart`、`SubagentStart`、`PreToolUse`、`PermissionRequest`、`PostToolUse`、`PreCompact`、`PostCompact`、`UserPromptSubmit`、`SubagentStop`、`Stop`（**注意：Codex 无 `Notification` 事件**）。**Aeroric 订阅 6 个**（`hooks.rs::CODEX_EVENTS`）：
...
- **配置格式**：`hooks.json` 或 config.toml 内联 `[[hooks.X]]` 二选一（同层两者并存会 merge 并告警）。Aeroric 用 config.toml 内联，marker 块 `# >>> aeroric-managed-begin ... <<<` 包裹，区域外用户内容按字符串切片保留。
...
同一语义在两个 agent 的原始 stdin JSON 里字段名可能不同，`aeroric-hook.mjs::pick()` 按优先级多 key 兜底：

| 语义 | Claude 字段 | Codex 字段 | aeroric-hook.mjs 兜底顺序 |
|------|------------|-----------|------------------------|
...
### 3.4 Aeroric 的 event → task-status 映射
...
| 维度 | Claude | Codex |
|------|--------|-------|
| 配置入口 | 命令行 `--settings <~/.aeroric/hooks/claude-settings.json>` | 内联注入 `~/.codex/config.toml` marker 块 |
...
| **Codex hook 未 trust** | Codex 静默 skip 该 hook | `events.jsonl` 为空 → session 发现/状态全失效。Aeroric 用 `--dangerously-bypass-hook-trust` 规避；**flag 必须在 `--`/`resume` 前**，否则 Codex 不识别。验证法：跑 codex 任务看 `~/.aeroric/events/<task_id>/events.jsonl` 是否有行 |
...
| 脚本无 `AERORIC_TASK_ID`/`AERORIC_EVENT_DIR` | `aeroric-hook.mjs` 立即 `exit 0` | 用户手动跑 agent 时零副作用（守卫设计） |
| 脚本内部任何异常 | catch 吞掉，`exit 0` | 绝不让 hook 失败阻塞 agent |
| **Windows 报 `hook exited with code 1`** | Node 读管道 stdin 到 EOF 抛 `EOF: end of file, read` 的 `'error'` 事件，无监听器 → 未捕获异常 → 进程 exit 1 | Unix 干净触发 `'end'`，Windows 抛 `'error'`（agent 写完 payload 关闭管道触发）。修复：`aeroric-hook.mjs` 必须给 `process.stdin.on('error', finish)` + `process.on('uncaughtException', finish)`（finish 幂等，用已收 payload 落盘并 `exit 0`）。脚本落盘逻辑全在 try/catch 内，exit 1 永远来自 stdin 流的未捕获 error，而非落盘逻辑 |
...
1. 在 `hooks.rs` 加 `<AGENT>_EVENTS` 常量，仅订阅 Aeroric 需要的语义事件（session 注册 / 等输入 / 复位）。
2. 在 `aeroric-hook.mjs::pick()` 的兜底 key 列表补该 agent 的字段名 + 兜底环境变量。
...
Aeroric 早期版本曾直接向用户 `~/.claude/settings.json` 内联注入带 `_nezha_managed` marker 的条目；现已改为命令行 `--settings` 传自有文件，`uninject_claude_settings_at` 仅用于清理旧版残留。Codex 侧曾因未处理 hook trust（hook 被静默 skip）+ `session_id` 字段名错（只读 `session_id` 未兜底 `conversation_id`）导致链路失效，已分别用 `--dangerously-bypass-hook-trust` 和多 key `pick()` 修复。cmux（同类 AI-agent 终端）的参考做法是 Codex 预置 `trusted_hash`（不降级安全），Aeroric 选了更简单的 bypass flag。

---

**相关文档：**

- [终端渲染与选区卡顿排查](../xterm/rendering-and-selection-lag.md) — 同属 Aeroric 桌面壳的独立技术线（无直接依赖）
