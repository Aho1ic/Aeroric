 ...
 * 语义:input_required / done 触发本地通知;同一任务同一状态只报
...
const NOTIFY_STATUSES = new Set(["input_required", "done"]);
...
export class TaskNotificationGate {
  ...
  evaluate(taskId: string, status: string, taskName?: string): TaskNotification | null {
    ...
    const title = status === "input_required" ? t("notify.inputRequired") : t("notify.done");
