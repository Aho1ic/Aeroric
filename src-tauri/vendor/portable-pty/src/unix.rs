...
/// stdio descriptors and mark them close-on-exec.
...
///
/// This function enumerates the open file descriptors in the current process
/// and marks each fd numbered 3 or higher with `FD_CLOEXEC`.
///
/// Do not close those descriptors directly from `pre_exec`: Rust's
/// `std::process::Command` owns an internal pipe used by the forked child to
/// report an `exec` failure to the parent. Closing that pipe makes the child
/// abort with `output.write(&bytes).is_ok()` and makes the parent believe the
/// spawn succeeded. `FD_CLOEXEC` gives us the intended leak protection while
/// keeping the error pipe alive until `exec` either succeeds or reports its
/// real error.
///
/// The implementation relies on `/dev/fd` being available to provide the
/// list of open fds. Any errors in enumerating or updating the fds are
/// silently ignored.
pub fn mark_random_fds_cloexec() {
...
mod tests {
    ...
    #[test]
    fn spawn_reports_exec_failure_to_the_parent() {
        let (_master, slave) = openpty(PtySize {
            rows: 24,
            cols: 80,
            pixel_width: 0,
            pixel_height: 0,
        ...
        .expect("open pty");
        let command = CommandBuilder::new("/aeroric/definitely/missing/agent");

        match slave.spawn_command(command) {
            Ok(mut child) => {
                let _ = child.kill();
                panic!("missing executable must fail during spawn");
            }
            Err(error) => {
