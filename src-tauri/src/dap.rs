...
// 段内全是私有 fn(没有 pub / pub(crate)),所以不需要额外的 re-export。
use paths::*;
use remote::*;
...
use protocol::{
    cdp_error_message, cdp_exception_message, debug_adapter_request_success,
    debug_adapter_response_error, debug_evaluate_result_from_remote_object,
    parse_debug_adapter_evaluate_result, parse_debug_adapter_stack_frames,
    parse_debug_adapter_variable_reference, parse_debug_adapter_variables,
    parse_debug_variables_from_properties, read_debug_adapter_message, write_debug_adapter_message,
};
...
mod tests {
    use super::*;
    // `unique_test_dir` 跟着 11 个路径用例搬去了 `paths.rs` 的模块层,
    // 这儿还有 2 个断点解析用例在用,取回来 —— 保持一份定义。
    use super::paths::unique_test_dir;
    use std::fs;
    use std::io::Cursor;
    use std::path::Path;
