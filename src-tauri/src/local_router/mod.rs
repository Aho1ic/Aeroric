...
pub struct RouterUpstreams {
    pub claude: Option<UpstreamTarget>,
    pub codex: Option<UpstreamTarget>,
}

impl RouterUpstreams {
    pub fn target(&self, agent: RouterAgent) -> Option<&UpstreamTarget> {
        match agent {
            RouterAgent::Claude => self.claude.as_ref(),
            RouterAgent::Codex => self.codex.as_ref(),
        }
    }
}
...
impl RouterRuntimeConfig {
    ...
    pub fn validate(&self) -> Result<SocketAddr, RouterError> {
        validate_listen_address(&self.listen_address, self.port).and_then(|listen_addr| {
            for target in [&self.upstreams.claude, &self.upstreams.codex]
                .into_iter()
                .flatten()
            {
...
impl LocalRouterState {
    ...
    pub fn with_database_path(database_path: PathBuf) -> Result<Self, RouterError> {
        let client = reqwest::Client::builder()
            ...
            .map_err(|_| {
                RouterError::new(
                    RouterErrorCode::InvalidConfig,
                    "failed to initialize local router HTTP client",
                )
            })?;
        Ok(Self {
            client,
...
fn validate_listen_address(address: &str, port: u16) -> Result<SocketAddr, RouterError> {
    ...
    let ip = address.parse::<IpAddr>().map_err(|_| {
        RouterError::invalid_config(
            "local router address must be localhost or a loopback IP address",
        )
    })?;
    if !ip.is_loopback() {
        return Err(RouterError::invalid_config(
            "local router can only listen on a loopback address",
        ));
    }
...
fn reject_router_loop(target: &UpstreamTarget, listen_addr: SocketAddr) -> Result<(), RouterError> {
    let is_loopback = match target.base_url.host() {
        Some(Host::Ipv4(address)) => address.is_loopback(),
        Some(Host::Ipv6(address)) => address.is_loopback(),
        Some(Host::Domain(domain)) => domain.eq_ignore_ascii_case("localhost"),
        None => false,
    };
    if is_loopback && target.base_url.port_or_known_default() == Some(listen_addr.port()) {
        return Err(RouterError::invalid_config(
            "upstream base URL points back to the local router listener",
        ));
    }
    Ok(())
}
...
mod tests {
    ...
    #[test]
    fn listener_validation_only_accepts_loopback_and_unprivileged_ports() {
        ...
        assert!(validate_listen_address("0.0.0.0", 43123).is_err());
        assert!(validate_listen_address("192.168.1.2", 43123).is_err());
