...
/// (`llm-pi-ai`,OpenAI 兼容端点;key 经 `DEEPSEEK_API_KEY` env 引用解析)。
...
pub(crate) fn ensure_dsh_home_at(home: &Path) -> Result<(), String> {
    ...
    if !settings.exists() {
        fs::write(&settings, SETTINGS_TEMPLATE)
    ...
    if !user_patch.exists() {
        fs::write(&user_patch, USER_PATCH_TEMPLATE)
    ...
    if current_version != Some(MANAGED_PATCH_VERSION) {
        crate::storage::atomic_write(&managed, &managed_patch_content(&sessions_root))
