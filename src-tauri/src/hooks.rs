...
fn hook_command(node_path: &str, script: &str) -> String {
    #[cfg(windows)]
    {
    ...
    #[cfg(not(windows))]
    {
        ...
        format!("{} {}", quote(node_path), quote(script))
    }
}
...
fn build_codex_block(node_path: &str, script: &str) -> String {
    ...
    for event in CODEX_EVENTS {
        ...
        // toml_quote 负责把命令中的引号与路径反斜杠转义成合法 TOML。
        out.push_str(&format!(
            "command = {}\n",
            toml_quote(&hook_command(node_path, script))
        ));
        out.push('\n');
    }
...
fn family_hooks_usable(
    ...
    match family {
        ...
        // dsh/omp 不走 claude/codex 的 hook 机制(改写用户 settings.json /
        // config.toml 的那一套):dsh 状态由自身 WebUI 通道上报;omp 用装在托管
        // home 里的原生 omp hook(见 `omp_home::ensure_omp_home_for`),事件写进
        // 同一份 events.jsonl,由 event_watcher 消费。
...
mod tests {
    ...
    fn claude_settings_value_has_all_events_no_scalar_keys() {
        ...
        for event in CLAUDE_EVENTS {
            ...
            assert!(cmd.contains("/script.mjs"));
        }
    }
