...
use approval::*;
use parse::*;
// `parse_session_lines` 原本是 `pub(crate)`,`remote/session_push.rs:111` 写的是
// `crate::session::parse_session_lines`。glob import 不替父模块对外转发可见性,
// 所以按原可见性显式 re-export 一手,调用点不用改。
...
pub(crate) fn validate_session_path_for(
    ...
    let allowed_roots: Vec<PathBuf> = match family {
        ...
        crate::app_settings::AgentFamily::Dsh => crate::session_dsh::dsh_session_allowed_roots()
            ...
            .filter_map(|p| p.canonicalize().ok())
            .collect(),
        // omp:托管 home(agent-homes/*/sessions)+ 用户自有 ~/.omp/agent/sessions。
        crate::app_settings::AgentFamily::Omp => crate::session_omp::omp_session_allowed_roots()
            .into_iter()
            .filter_map(|p| p.canonicalize().ok())
            .collect(),
    };
