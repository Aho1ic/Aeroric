...
const event = (type: string, seq: number, time: number, data: unknown): DshSessionEvent => ({
  type,
  seq,
  time,
  data,
...
describe("DSH advanced session projections", () => {
  ...
  it("projects workflow phases and recurring schedules", () => {
    const features = projectDshSessionEvents([
      ...
      event("tool-workflow/agent-start", 2, 2, {
        runId: "run-1",
        seq: 1,
        label: "Build",
        phase: "Build",
      ...
      event("schedule/change", 6, 6, {
        operation: "dispatch",
        id: "schedule-1",
        acceptedAt: "2999-01-01T00:05:00.000Z",
      }),
    ]);
    expect(features.workflows[0]).toMatchObject({
      name: "Release",
      status: "completed",
      phases: { Build: { members: [{ status: "completed", childId: "child-1" }] } },
    });
    expect(features.schedules[0]).toMatchObject({
      id: "schedule-1",
      state: "scheduled",
      scheduledAt: "2999-01-01T00:10:00.000Z",
    });
  });
  ...
  it("deduplicates live and history events by sequence", () => {
    ...
    expect(
      mergeDshSessionEvents([original], [replacement, event("turn/end", 2, 3, { turn: 1 })]),
    ).toEqual([replacement, event("turn/end", 2, 3, { turn: 1 })]);
  });

  it("unwraps a history page delivered as HistoryEntry wrappers", () => {
    // session.history returns `{ event, view? }` wrappers, not bare events; a
    // reader that looks for `type` on the wrapper drops the whole page and every
    // insight panel renders empty for a session that was not watched live.
    const page = readDshHistoryPage([
      { event: event("turn/start", 1, 10, { turn: 1 }) },
      {
        event: event("tool/call", 2, 20, { turn: 1, name: "bash" }),
        view: { for: "call", view: { card: "terminal", title: "pnpm test", cwd: "/repo" } },
      },
      { event: { missing: "type" } },
      "not an entry",
    ]);
    expect(page.events.map((entry) => entry.type)).toEqual(["turn/start", "tool/call"]);
    expect(page.views).toEqual({
      2: { for: "call", view: { card: "terminal", title: "pnpm test", cwd: "/repo" } },
    });
  });

  it("reads a page of bare events, so an unwrapped deployment behaves the same", () => {
    const page = readDshHistoryPage([event("turn/start", 1, 10, { turn: 1 })]);
    expect(page.events).toEqual([event("turn/start", 1, 10, { turn: 1 })]);
    expect(page.views).toEqual({});
  });

  it("attaches render intents to the trajectory entry sharing their seq", () => {
    const intent: DshToolEventView = {
      for: "result",
      view: { card: "terminal", output: "ok", exitCode: 0 },
    };
    const features = projectDshSessionEvents(
      [event("tool/call", 1, 10, { name: "bash" }), event("tool/result", 2, 20, {})],
      { 2: intent },
    );
