import type React from "react";

export const layout = {
  root: {
    display: "flex",
    flexDirection: "column",
    height: "100%",
    background: "var(--bg-root)",
    overflow: "hidden",
  },
  welcomeBody: {
    height: "100%",
    display: "flex",
    overflow: "hidden",
    background: "var(--bg-shell)",
    position: "relative",
    isolation: "isolate",
  },
  welcomeRecursiveBackground: {
    position: "absolute",
    inset: 0,
    overflow: "hidden",
    pointerEvents: "none",
    zIndex: 0,
    background: "var(--bg-shell)",
  },
  welcomeMain: { flex: 1, display: "flex", overflow: "hidden", position: "relative", zIndex: 1 },
  sidebar: {
    width: 138,
    flexShrink: 0,
    // 首页动画画布是它的兄弟层且铺满整个 welcomeBody,画布每帧全量重绘都会脏掉
    // 这一层的 backdrop,导致 60Hz 重新模糊。改成不透明实色。
    background: "var(--bg-sidebar)",
    borderRight: "1px solid var(--border-dim)",
    display: "flex",
    flexDirection: "column",
    padding: "12px 7px 10px",
  },
  sidebarBrand: {
    display: "flex",
    alignItems: "center",
    gap: 7,
    padding: "2px 4px 12px",
    marginBottom: 10,
    borderBottom: "1px solid var(--border-dim)",
  },
  sidebarBrandIcon: {
    width: 34,
    height: 34,
    borderRadius: "var(--radius-md)",
    background: "var(--brand-mark-gradient)",
    padding: 2,
    boxShadow: "0 6px 16px var(--brand-mark-glow)",
    display: "flex",
    alignItems: "center",
    justifyContent: "center",
    overflow: "hidden",
    flexShrink: 0,
  },
  sidebarBrandBadge: {
    display: "flex",
    alignItems: "flex-end",
    justifyContent: "center",
    width: "100%",
    height: "100%",
    borderRadius: "var(--radius-sm)",
    background: "var(--brand-mark-inner-bg)",
    color: "var(--brand-mark-fg)",
    fontSize: 12,
    fontWeight: 800,
    letterSpacing: 0.6,
    paddingBottom: 4,
  },
  sidebarBrandTitle: {
    fontSize: 13,
    fontWeight: 700,
    color: "var(--text-primary)",
    lineHeight: 1.2,
  },
  sidebarBrandMeta: { marginTop: 1, fontSize: 10.5, color: "var(--text-muted)" },
  sidebarNav: { display: "flex", flexDirection: "column", gap: 4 },
  sidebarSectionTitle: {
    fontSize: 10,
    fontWeight: 700,
    color: "var(--text-hint)",
    letterSpacing: 0.7,
    padding: "0 8px 4px",
    textTransform: "uppercase" as const,
  },
  sidebarItem: {
    display: "flex",
    alignItems: "center",
    padding: "8px 7px",
    borderRadius: "var(--radius-md)",
    cursor: "pointer",
    fontWeight: 500,
    transition: "background 0.18s ease, color 0.18s ease",
    border: "1px solid transparent",
  },
  sidebarItemMeta: {
    marginLeft: "auto",
    minWidth: 22,
    height: 22,
    padding: "0 7px",
    borderRadius: 999,
    background: "var(--badge-muted-bg)",
    color: "var(--text-muted)",
    fontSize: 11,
    fontWeight: 700,
    display: "inline-flex",
    alignItems: "center",
    justifyContent: "center",
  },
  sidebarFooter: {
    marginTop: "auto",
    paddingTop: 8,
    borderTop: "1px solid var(--border-dim)",
    display: "flex",
    justifyContent: "flex-start",
  },
  sidebarFooterActions: { display: "flex", alignItems: "center", gap: 4 },
  sidebarIconBtn: {
    background: "none",
    border: "none",
    cursor: "pointer",
    padding: 4,
    borderRadius: 6,
    display: "flex",
    alignItems: "center",
    position: "relative",
    overflow: "visible",
    opacity: 0.5,
  },
  // 首页左下角四个按钮（版本发布 / 应用设置 / 主题切换 / 用量）专用。
  // 刻意不改共享的 sidebarIconBtn：那条样式还被 ProjectRail 侧栏复用，改基础值会连带放大项目页按钮。
  // overflow/position 必须保留，否则 NotificationBell 的未读红点会被裁掉。
  sidebarFooterIconBtn: {
    background: "none",
    border: "none",
    cursor: "pointer",
    padding: 7,
    borderRadius: 8,
    display: "flex",
    alignItems: "center",
    justifyContent: "center",
    minWidth: 30,
    minHeight: 30,
    position: "relative",
    overflow: "visible",
    opacity: 0.5,
  },
  // 首页项目区直接叠在递归动画画布上。这里刻意不加 background 底色与 backdrop-filter：
  // 半透明蒙层 + 24px 模糊会把画布压成一层虚影，动画等于看不见。需要可读性的地方
  // （搜索框、项目行、分组标题）各自有实心/描边样式，不依赖整块面板的毛玻璃。
  welcomePane: {
    flex: 1,
    display: "flex",
    flexDirection: "column",
    overflow: "hidden",
    background: "transparent",
  },
  searchRow: {
    display: "flex",
    alignItems: "center",
    gap: 14,
    padding: "18px 22px 12px",
    borderBottom: "1px solid var(--border-dim)",
    background: "transparent",
    flexShrink: 0,
  },
  searchBox: {
    flex: 1,
    display: "flex",
    alignItems: "center",
    gap: 12,
    minWidth: 0,
    padding: "12px 16px",
    background: "var(--bg-input)",
    border: "1px solid var(--border-medium)",
    borderRadius: "var(--radius-lg)",
    transition: "box-shadow 0.18s ease, border-color 0.18s ease",
    boxShadow: "var(--shadow-xs)",
  },
  searchInput: {
    flex: 1,
    border: "none",
    outline: "none",
    background: "transparent",
    color: "var(--text-primary)",
    fontSize: 15,
    fontWeight: 500,
  },
  actionRow: { display: "flex", alignItems: "center", gap: 10, flexShrink: 0 },
  primaryActionBtn: {
    display: "inline-flex",
    alignItems: "center",
    gap: 8,
    height: 38,
    padding: "0 16px",
    background: "var(--primary-action-bg)",
    color: "var(--primary-action-fg)",
    border: "1px solid var(--primary-action-bg)",
    borderRadius: "var(--radius-md)",
    fontSize: 13.5,
    fontWeight: 700,
    cursor: "pointer",
    boxShadow: "var(--shadow-xs)",
  },
  secondaryActionBtn: {
    display: "inline-flex",
    alignItems: "center",
    gap: 8,
    height: 38,
    padding: "0 14px",
    background: "var(--bg-card)",
    color: "var(--text-secondary)",
    border: "1px solid var(--border-dim)",
    borderRadius: "var(--radius-md)",
    fontSize: 13,
    fontWeight: 650,
    cursor: "pointer",
  },
  projectSectionHeader: { padding: "14px 22px 6px", flexShrink: 0 },
  projectSectionTitle: { fontSize: 14, fontWeight: 700, color: "var(--text-primary)" },
  projectSectionCaption: { marginTop: 4, fontSize: 12, color: "var(--text-muted)" },
  projectList: { flex: 1, overflowY: "auto", padding: "4px 22px 18px" },
  projectGroupSection: { marginBottom: 10 },
  projectGroupHeader: {
    width: "100%",
    height: 34,
    display: "flex",
    alignItems: "center",
    justifyContent: "space-between",
    gap: 12,
    padding: "0 10px",
    border: "none",
    borderBottom: "1px solid color-mix(in srgb, #16a34a 45%, var(--border-dim))",
    background: "transparent",
    color: "var(--text-muted)",
    cursor: "pointer",
    fontFamily: "var(--font-ui)",
    fontSize: 11.5,
    fontWeight: 700,
    textAlign: "left" as const,
  },
  projectGroupHeaderLabel: {
    minWidth: 0,
    display: "inline-flex",
    alignItems: "center",
    gap: 6,
  },
  projectGroupHeaderName: {
    minWidth: 0,
    overflow: "hidden",
    textOverflow: "ellipsis",
    whiteSpace: "nowrap" as const,
  },
  projectGroupHeaderCount: {
    flexShrink: 0,
    color: "var(--text-hint)",
    fontVariantNumeric: "tabular-nums",
  },
  projectItem: {
    display: "flex",
    alignItems: "center",
    gap: 14,
    width: "100%",
    minHeight: 72,
    padding: "12px 14px",
    border: "1px solid transparent",
    borderRadius: "var(--radius-lg)",
    cursor: "pointer",
    textAlign: "left",
    fontFamily: "var(--font-ui)",
    transition: "background 0.16s ease, border-color 0.16s ease, box-shadow 0.16s ease",
  },
  projectName: {
    fontSize: 14,
    fontWeight: 650,
    color: "var(--text-primary)",
    overflow: "hidden",
    textOverflow: "ellipsis",
    whiteSpace: "nowrap",
  },
  projectNameInput: {
    width: "100%",
    height: 26,
    border: "1px solid transparent",
    borderRadius: 6,
    background: "color-mix(in srgb, var(--bg-panel) 46%, transparent)",
    backdropFilter: "blur(14px) saturate(1.12)",
    WebkitBackdropFilter: "blur(14px) saturate(1.12)",
    color: "var(--text-primary)",
    fontFamily: "var(--font-ui)",
    fontSize: 14,
    fontWeight: 650,
    padding: "0 7px",
    outline: "none",
    boxShadow: "inset 0 0 0 1px color-mix(in srgb, var(--bg-panel) 36%, transparent)",
    textTransform: "none" as const,
  },
  projectNameEditing: {
    display: "inline-block",
    maxWidth: "100%",
    minWidth: 24,
    outline: "none",
    cursor: "text",
    overflow: "hidden",
    textOverflow: "ellipsis",
    whiteSpace: "nowrap",
    borderBottom: "1px solid var(--border-focus)",
  },
  projectMeta: {
    marginTop: 4,
    fontSize: 11.5,
    color: "var(--text-muted)",
    overflow: "hidden",
    textOverflow: "ellipsis",
    whiteSpace: "nowrap",
  },
  branchBadge: {
    display: "inline-flex",
    alignItems: "center",
    gap: 4,
    fontSize: 10.5,
    color: "var(--text-secondary)",
    background: "var(--bg-subtle)",
    border: "1px solid var(--border-dim)",
    padding: "4px 8px",
    borderRadius: 999,
    flexShrink: 0,
  },
  projectTag: {
    fontSize: 10.5,
    fontWeight: 700,
    color: "var(--text-hint)",
    letterSpacing: 0.7,
    flexShrink: 0,
  },
  projectPinBtn: {
    display: "inline-flex",
    alignItems: "center",
    gap: 4,
    marginLeft: 8,
    padding: "4px 8px",
    fontSize: 10.5,
    fontWeight: 600,
    borderRadius: 999,
    border: "1px solid transparent",
    cursor: "pointer",
    flexShrink: 0,
    whiteSpace: "nowrap",
    fontFamily: "var(--font-ui)",
    transition: "color 0.15s ease, border-color 0.15s ease, background 0.15s ease",
  },
  projectPinBtnPinned: {
    background: "var(--bg-subtle)",
    borderColor: "var(--border-dim)",
    color: "var(--text-secondary)",
  },
  projectPinBtnHidden: {
    background: "transparent",
    borderColor: "var(--border-dim)",
    color: "var(--text-hint)",
  },
  railHiddenIcon: {
    marginLeft: "auto",
    flexShrink: 0,
  },
  emptyState: {
    display: "flex",
    flexDirection: "column",
    alignItems: "center",
    justifyContent: "center",
    height: "100%",
    padding: 32,
    textAlign: "center",
  },
  emptyOpenBtn: {
    display: "inline-flex",
    alignItems: "center",
    gap: 7,
    padding: "10px 18px",
    background: "var(--bg-card)",
    color: "var(--text-secondary)",
    border: "1px solid var(--border-medium)",
    borderRadius: "var(--radius-lg)",
    fontSize: 13,
    fontWeight: 600,
    cursor: "pointer",
    boxShadow: "var(--shadow-sm)",
  },
  projectBody: { flex: 1, display: "flex", overflow: "hidden" },
  mainContent: {
    flex: 1,
    display: "flex",
    flexDirection: "column",
    overflow: "hidden",
    background: "var(--bg-panel)",
  },
  placeholderView: { flex: 1, display: "flex", alignItems: "center", justifyContent: "center" },
} satisfies Record<string, React.CSSProperties>;
